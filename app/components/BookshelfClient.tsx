      ...
      <div className="container mx-auto px-4 py-6">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
          ...
          <aside className={`md:col-span-1 space-y-6 ${isMobileMenuOpen ? "block order-1" : "hidden md:block order-2 md:order-1"}`}>
            ...
            <div className={`bg-white rounded-lg shadow-lg border-4 border-black p-4 ${isAuthorsOpen ? '' : 'pb-0'}`}>
              <button
                data-tour="authors"
                onClick={() => setIsAuthorsOpen(!isAuthorsOpen)}
                className={`w-full flex items-center justify-between text-red-600 font-bold text-sm uppercase tracking-wide hover:bg-orange-50 px-4 py-3 rounded transition-colors ${isAuthorsOpen ? 'mb-3' : 'mb-0'}`}
              >
                <div className="flex items-center gap-2">
            ...
            {userState.suggestNewAuthors && (
              <div className={`bg-white rounded-lg shadow-lg border-4 border-black p-4 ${isRecommendationsOpen ? '' : 'pb-0'}`}>
                <button
                  data-tour="recommendations"
                  onClick={() => setIsRecommendationsOpen(!isRecommendationsOpen)}
                  className={`w-full flex items-center justify-between text-red-600 font-bold text-sm uppercase tracking-wide hover:bg-orange-50 px-4 py-3 rounded transition-colors ${isRecommendationsOpen ? 'mb-3' : 'mb-0'}`}
                >
                  <div className="flex items-center gap-2">
            ...
            <div className={`bg-white rounded-lg shadow-lg border-4 border-black p-4 ${isPreferencesOpen ? '' : 'pb-0'}`}>
              <button
                data-tour="settings"
                onClick={() => setIsPreferencesOpen(!isPreferencesOpen)}
                className={`w-full flex items-center justify-between text-red-600 font-bold text-sm uppercase tracking-wide hover:bg-orange-50 px-4 py-3 rounded transition-colors ${isPreferencesOpen ? 'mb-3' : 'mb-0'}`}
              >
                <div className="flex items-center gap-2">
